...
const setupResult = () => {
  if (vataScore > pittaScore && vataScore > kaphaScore) {
    // vata が一番高い
    $result.innerText = result[0].type
    $resultDetail.innerText = result[0].detail
  } else if (pittaScore > vataScore && pittaScore > kaphaScore) {
    // pitta が一番高い
    $result.innerText = result[1].type
    $resultDetail.innerText = result[1].detail
  } else if (kaphaScore > vataScore && kaphaScore > pittaScore) {
    // kapha が一番高い
    $result.innerText = result[2].type
    $resultDetail.innerText = result[2].detail
  } else if (vataScore == pittaScore) {
    // vata と pitta
    $result.innerText = result[0].type
    $resultDetail.innerText = result[0].detail
    $result_2.innerText = result[1].type
    $resultDetail_2.innerText = result[1].detail
  } else if (vataScore == kaphaScore) {
    // vata と kapha
    $result.innerText = result[0].type
    $resultDetail.innerText = result[0].detail
    $result_2.innerText = result[2].type
    $resultDetail_2.innerText = result[2].detail
